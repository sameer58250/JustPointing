...
const Home = (props) => {
    ...
    const joinSession = () => {
        ...
        SessionManager.StartSession(sessionId)
            ...
            .catch((error) => {
                props.failure(
                    "Session is not found. Please try with different session Id."
                );
            });
    };
    ...
    return (
        <div>
            <div className="app-title">
                <p>
                    Collaborate to size your items and make agile development
                    easy
                </p>
            </div>
            <div className="home">
                ...
                <input
                    type="text"
                    id="session-id-input"
                    onChange={sessionIdInputChange}></input>
                <button onClick={joinSession} disabled={!sessionIdInput}>
                    {" "}
                    Join Session
                </button>
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        failure: (errorText) =>
            dispatch(actions.create_session_failure(errorText)),
        sessionStarted: () => dispatch(actions.session_started()),
        openLoginPopup: (openLogin) =>
            dispatch(actions.open_login_popup(openLogin)),
    };
}
